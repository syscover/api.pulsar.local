...
@section('content')
    ...
    <div class="card-columns">
        ...
        <!--
            Show products With fixed values from custom fields.
            In this case highlight exist values
        -->
        @foreach($products as $product)
            <div class="card">
                <!-- slider -->
                ...
                <div class="card-block">
                    ...
                    @if($product->fieldGroup !== null)
                        <p class="card-text">
                            <div class="row">
                                <div class="col-1 {{ $product->whereChildrenProperty('size', 'XS')->count() > 0? 'aviable' : null }}">XS</div>
                                <div class="col-1 {{ $product->whereChildrenProperty('size', 'S')->count() > 0? 'aviable' : null }}">S</div>
                                <div class="col-1 {{ $product->whereChildrenProperty('size', 'M')->count() > 0? 'aviable' : null }}">M</div>
                                <div class="col-1 {{ $product->whereChildrenProperty('size', 'L')->count() > 0? 'aviable' : null }}">L</div>
                                <div class="col-1 {{ $product->whereChildrenProperty('size', 'XL')->count() > 0? 'aviable' : null }}">XL</div>
                            </div>
                            <div class="row">
                                <div class="col-1 {{ $product->whereChildrenProperty('color', 'FFFFFF')->count() > 0? 'aviable' : null }}">W</div>
                                <div class="col-1 {{ $product->whereChildrenProperty('color', '000000')->count() > 0? 'aviable' : null }}">B</div>
                                <div class="col-1 {{ $product->whereChildrenProperty('color', 'FFFF00')->count() > 0? 'aviable' : null }}">Y</div>
                                <div class="col-1 {{ $product->whereChildrenProperty('color', '0000FF')->count() > 0? 'aviable' : null }}">B</div>
                            </div>
                        </p>
                    @endif
        ...
        <!--
            Show products with exist values from custom fields in select.
        -->
        @foreach($products as $product)
            <div class="card">
                <!-- slider -->
                            ...
                            @foreach($product->fieldGroup->fields as $field)
                                @if($field->values->where('lang_id', user_lang())->count() > 0)
                                    ...
                                    <select class="custom-select">
                                        <option>{{ $field->labels[user_lang()] }}</option>
                                        @foreach($field->values->where('lang_id', user_lang()) as $value)
                                            @if($product->whereChildrenProperty($field->name, $value->id)->count() > 0)
                                                <option value="{{ $value->id }}">
                                                    {{ $value->name }}
                                                </option>
                                            @endif
